...
def init_state():
    defaults = {
        ...
        "pnl_series": [],        # list of (timestamp, cumulative_pnl)
...
def compute_edge(buy_yes, buy_no, clob_fee=0.00075, gas_usd=0.5, swap_spread=0.0002,
                 buffer_bps=10, trade_size_usd=500):
...
def simulate_execution(buy_yes, buy_no, net_edge, params):
    ...
    trade_size = random.uniform(params["min_trade_usd"], min(params["max_trade_usd"], 2000))
...
def fetch_markets(min_volume=100_000, limit=20):
    """Fetch top binary markets from Gamma API (public, no auth)."""
    try:
        ...
        markets = []
        for m in data:
            # Filter binary YES/NO markets only
...
def fetch_orderbook(token_id: str):
    ...
    try:
        r = requests.get(f"{CLOB_API}/book", params={"token_id": token_id}, timeout=6)
...
def synth_orderbook(base_price):
    ...
    asks = [{"price": str(round(ask + i*0.003, 4)),
             "size": str(round(random.uniform(50, 500), 1))} for i in range(5)]
    bids = [{"price": str(round(bid - i*0.003, 4)),
             "size": str(round(random.uniform(50, 500), 1))} for i in range(5)]
...
with st.sidebar:
    ...
    mode = st.toggle("🌐 Live Polymarket Data", value=st.session_state.live_mode,
    ...
    st.session_state.refresh_interval_sec = st.slider("Refresh interval (sec)", 3, 60, 
                                                        st.session_state.refresh_interval_sec)
    st.session_state.min_volume_usd = st.number_input("Min Volume ($)", 0, 10_000_000,
                                                       int(st.session_state.min_volume_usd), 
                                                       step=50_000)
    st.session_state.max_markets = st.slider("Max markets to monitor", 3, 25,
                                              st.session_state.max_markets)

    st.divider()
    st.markdown("**💰 Cost Model**")
    st.session_state.profit_threshold_bps = st.slider("Profit threshold (bps)", 5, 200,
                                                        st.session_state.profit_threshold_bps)
    st.session_state.clob_fee_pct = st.number_input("CLOB fee (%)", 0.0, 1.0,
                                                      st.session_state.clob_fee_pct, 0.005,
                                                      format="%.3f")
    st.session_state.gas_merge_usd = st.number_input("Gas/merge cost ($)", 0.0, 5.0,
                                                       st.session_state.gas_merge_usd, 0.1,
                                                       format="%.2f")
    st.session_state.swap_spread_pct = st.number_input("Swap spread (%)", 0.0, 1.0,
                                                         st.session_state.swap_spread_pct, 0.005,
                                                         format="%.3f")
    st.session_state.buffer_bps = st.slider("Safety buffer (bps)", 0, 50,
                                              st.session_state.buffer_bps)

    st.divider()
    st.markdown("**📐 Execution**")
    c1, c2 = st.columns(2)
    with c1:
        st.session_state.min_trade_usd = st.number_input("Min trade $", 10, 1000,
                                                           int(st.session_state.min_trade_usd), 10)
    with c2:
        st.session_state.max_trade_usd = st.number_input("Max trade $", 100, 50000,
                                                           int(st.session_state.max_trade_usd), 100)
    st.session_state.slippage_pct = st.number_input("Max slippage (%)", 0.0, 5.0,
                                                      st.session_state.slippage_pct, 0.1,
                                                      format="%.1f")

    st.divider()
    if st.button("🗑 Reset Session"):
        for k in ["trade_history","pnl_series","logs","total_pnl","total_trades","total_opps","edge_history"]:
...
with col_h2:
    ...
    status_html = (
        ...
        if is_live else
        '<span class="status-dot dot-gray"></span> SYNTHETIC SIM'
    )
    st.markdown(f"""
...
with tabs[0]:
    ...
    with c2:
        pnl_color = "green" if st.session_state.total_pnl >= 0 else "red"
        st.markdown(f"""
    ...
    if results:
        ...
        hot_count = sum(1 for r in results if r.get("net_edge") and r["net_edge"] > 0)

        # Build styled dataframe
        rows = []
        for r in results:
            if r["yes_ask"] is None:
                badge = '<span class="opportunity-badge badge-cold">NO DATA</span>'
                rows.append({
                    "Market": r["question"],
                    "YES Ask": "—", "NO Ask": "—",
                    "Raw Edge": "—", "Net Edge": "—",
                    "Status": r["status"], "Sim PnL": "$0.00",
        ...
        def style_row(row):
            styles = [""] * len(row)
            if "HOT" in str(row.get("Status", "")) or "EXEC" in str(row.get("Status", "")):
                styles = ["background-color: rgba(0,255,136,0.05); color: #00ff88"] * len(row)
            elif "WARM" in str(row.get("Status", "")):
                styles = ["background-color: rgba(245,158,11,0.04); color: #f59e0b"] * len(row)
            return styles
    ...
    if st.session_state.logs:
        ...
        for entry in list(st.session_state.logs)[:20]:
            css = "log-entry"
            if entry["level"] == "arb":  css += " arb"
            elif entry["level"] == "warn": css += " warn"
            elif entry["level"] == "err":  css += " err"
...
with tabs[1]:
    ...
    with mc3:
        color = "normal" if net_e > 0 else "inverse"
    ...
    if net_e > st.session_state.profit_threshold_bps / 10000:
        st.success(f"✅ OPPORTUNITY DETECTED — Net edge {net_e*100:.2f}% exceeds threshold {st.session_state.profit_threshold_bps}bps")
    elif raw_e > 0:
        st.warning(f"⚠️ Raw edge exists but fees consume it. Need {st.session_state.profit_threshold_bps}bps net.")
    else:
        st.error(f"❌ No arbitrage opportunity at current prices.")
...
with tabs[2]:
    ...
    if not st.session_state.trade_history:
        ...
        with col_a:
            ...
            fig_hist = px.histogram(
                edge_df, x="Net Edge (%)", nbins=20,
                color_discrete_sequence=["#00ff88"],
            )
            fig_hist.add_vline(x=st.session_state.profit_threshold_bps/100,
    ...
    if st.button("🔬 Run Parameter Sweep"):
        ...
        for thr_bps in thresholds:
            ...
            for _ in range(sweep_runs):
                ...
                for m in markets:
                    ...
                    if net > thr:
                        res = simulate_execution(by, bn, net, {
                            **DEFAULT_PARAMS,
                            **{k: st.session_state[k] for k in DEFAULT_PARAMS}
                        })
                        if res:
                            total_p += res["net_pnl"]
                            n_trades += 1
                            if res["net_pnl"] > 0: wins_s += 1
            sweep_results.append({
                "Threshold (bps)": thr_bps,
                "Trades": n_trades,
                "Total PnL ($)": round(total_p, 2),
        ...
        fig_sw.update_layout(
            ...
            margin=dict(l=40, r=60, t=20, b=40), height=320,
        )
...
# Fetch top binary markets (public, no key)
def fetch_markets(min_volume=100_000, limit=20):
...
def best_ask(ob):
    asks = ob.get("asks", [])
    prices = [float(o["price"]) for o in asks
              if float(o.get("size",0)) > 0]
    return min(prices) if prices else None
        """, language="python")

    with st.expander("🎯 Opportunity Check (_check_opportunity)"):
        st.code("""
def check_opportunity(yes_asks, no_asks, yes_bids, no_bids,
    ...
    buy_yes = effective["effective_buy_yes"]
    buy_no  = effective["effective_buy_no"]

    # Long arb: buy both sides, merge for $1
    long_cost = buy_yes + buy_no
    long_edge = 1.0 - long_cost  # PRE-FEE

    if long_edge > profit_threshold:
        return {"type": "LONG", "edge": long_edge,
                "buy_yes": buy_yes, "buy_no": buy_no}

    # Short arb: sell both sides
    sell_yes = effective["effective_sell_yes"]
    sell_no  = effective["effective_sell_no"]
    short_rev  = sell_yes + sell_no
    short_edge = short_rev - 1.0

    if short_edge > profit_threshold:
        return {"type": "SHORT", "edge": short_edge,
                "sell_yes": sell_yes, "sell_no": sell_no}

    return None  # no opportunity
        """, language="python")

    with st.expander("💰 Full Cost Model (After-Fee Edge)"):
        st.code("""
def compute_net_edge(buy_yes, buy_no,
                     clob_fee=0.00075,    # 0.075% per leg
                     gas_merge_usd=0.50,  # polygon gas
                     swap_spread=0.0002,  # USDC↔USDC.e
                     buffer_bps=10,       # safety
                     trade_size_usd=500):
    ...
    fees_clob  = clob_fee * 2  # both legs
    gas_frac   = gas_merge_usd / trade_size_usd
    buffer     = buffer_bps / 10_000

    net_edge = raw_edge - fees_clob - gas_frac - swap_spread - buffer
    return raw_edge, net_edge

# Formula from article:
# edge = 1.0 - (p_YES^buy + p_NO^buy)
#             - fees_clob - gas_merge - swap_spread - buffer
        """, language="python")

    with st.expander("🔄 Execution Simulation"):
        st.code("""
import random

def simulate_execution(buy_yes, buy_no, net_edge,
                        min_trade_usd=50, max_trade_usd=5000,
                        slippage_pct=0.5, gas_usd=0.5):
    # Random slippage (0 to max)
    slip = random.uniform(0, slippage_pct / 100)
    # Partial fill (75-100%)
    fill = random.uniform(0.75, 1.0)
    trade_usd = random.uniform(min_trade_usd, max_trade_usd)

    cost_per_share = buy_yes + buy_no + slip
    if cost_per_share >= 1.0:
        return None  # unprofitable after slippage

    shares    = (trade_usd * fill) / cost_per_share
    gross_pnl = shares * (1.0 - cost_per_share)
    net_pnl   = gross_pnl - gas_usd

    return {
        "shares": shares, "gross_pnl": gross_pnl,
        "net_pnl": net_pnl, "fill_pct": fill * 100,
    }
        """, language="python")

    with st.expander("📐 ArbitrageStrategy Class Structure (from article)"):
        st.code("""
class ArbitrageStrategy:
    \"\"\"Real-time arbitrage strategy (fee-adjusted).
    Ported from Jakub's 2026 Python framework.
    Production: uses WebSocket + TradingClient.
    This simulator: uses REST polling + simulation.
    \"\"\"
    def __init__(self, settings, api_client, trading_client, ws_client):
        self.profit_threshold = float(settings.arbitrage_profit_threshold)
        self.min_trade_usd    = float(settings.min_trade_usd)
        self.max_trade_usd    = float(settings.max_trade_usd)
        self.dry_run          = bool(settings.dry_run)
        self.stats = {
            "opportunities": 0, "executions": 0, "failed": 0,
            "edge_estimate_usdc": 0.0,  # cumulative pre-fee edge
        }

    async def _execute_long_arb(self, price_yes, price_no, edge):
        now = time.time()
        if now - self.last_execution_time < self.execution_cooldown:
            return
        self.stats["opportunities"] += 1
        usd_budget = self.min_trade_usd
        size = usd_budget / (price_yes + price_no)
        if self.dry_run:
            return  # paper trading stops here
        # Production: place two parallel BUY limit orders
        res_yes, res_no = await asyncio.gather(
            self.trade_executor.create_order(
                token_id=self.market_config["yes_token"],
                price=price_yes, size=size, side="BUY"),
            self.trade_executor.create_order(
                token_id=self.market_config["no_token"],
                price=price_no,  size=size, side="BUY"),
        )
        if res_yes.get("success") and res_no.get("success"):
            await asyncio.sleep(2)
            # Merge size = min(filled_yes, filled_no)
            await self.trade_executor.merge_positions(
                condition_id=self.market_config["condition_id"],
                amount=Decimal(str(round(size, 4)))
            )
            self.stats["executions"] += 1
            self.stats["edge_estimate_usdc"] += edge * size
        """, language="python")
